...
def run_trigger_eval(
    ...
    try:
        ...
        raise EvalError(
            code="E_LOAD_QUERIES",
            what=f"Failed to load queries from {queries_file}",
            why=str(e),
            fix="Ensure eval_queries.json exists and contains valid JSON",
        )

    if not queries:
        raise EvalError(
            code="E_NO_QUERIES",
            what="No trigger queries found",
            why=f"The queries file at {queries_file} is empty or contains no valid queries",
            fix="Add trigger queries to eval_queries.json, or run 'skillctl init' to generate a scaffold",
        )
    ...
    except (KeyError, EvalError) as e:
        raise EvalError(
            code="E_UNKNOWN_RUNNER",
            what=f"Unknown agent runner: {agent}",
            why=str(e),
            fix="Use --agent claude (default) or register a custom runner",
        )
